...
export default function NowPlaying() {
  const { activeEngines, stopEngine, stopNsdrSession, getElapsed, nsdrNarration } = useAudioEngine();
  const navigate = useNavigate();
  const [, forceUpdate] = useState(0);

  // Build list of running modules
  const running = [];
  if (activeEngines.focus) running.push('focus');
  if (nsdrNarration.active) running.push('nsdr');
  else if (activeEngines.nsdr) running.push('nsdr');
  ...
  return (
    <div style={{
      ...
      left: 0, right: 0, zIndex: 999, padding: '0 8px',
    }}>
      <div style={{
        maxWidth: 600, margin: '0 auto',
        background: 'rgba(18, 18, 24, 0.95)',
        backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
        borderRadius: 14, border: '1px solid #252530',
        ...
        {running.map(id => {
          ...
          const elapsed = id === 'nsdr' && nsdrNarration.active ? nsdrNarration.elapsed : getElapsed(id);

          return (
            <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
              <div style={{
